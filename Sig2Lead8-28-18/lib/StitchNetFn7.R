...
mol <- unique(mol)
mol <<- mol
group <- replace(mol, toupper(mol) %in% toupper(chemInput), "Input chemicals presented in Stitch")
group <- replace(group, toupper(group) %in% toupper(genes), "Gene KD")
group <- replace(group, !group %in% c("Input chemicals presented in Stitch", "Gene KD"), "Stitch connectors")
...
undetect_Node <- data.frame(label = stitch_undetect, id = stitch_undetect, group = "Undetected in Stitch Network", 
                            title = paste0('<a target= "_blank"', '', 'href=', '', 
...
visNetwork(molnodes, interScore) %>%
  ...
  visGroups(groupname = "Input chemicals presented in Stitch", shape = "ellipse",
  ...
  visGroups(groupname = "Stitch connectors", shape = "dot", size = 10,
            color = list(background = "ddd", border = "black", 
                         highlight = list(background = "caff44", 
                                          border = "red"))) %>%
  visGroups(groupname = "Undetected in Stitch Network", shape = "ellipse", 
